...
public class CheckPlacesServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // 1. Récupérer toutes les places occupées dans la table RESERVER pour cette date précise
        List<Integer> occupees = resDAO.getPlacesOccupeesParDate(idvoit, dateVoyage);

        // 2. Identifier les places du client actuel (utile pour le mode modification)
        ...
        // 3. Récupérer le nombre total de places de la voiture
        Voiture v = voitDAO.getById(idvoit);
        int total = (v != null) ? v.getNbrplace() : 0;

        StringBuilder html = new StringBuilder();

        // Style commun pour les rectangles (dimensions adaptées pour "Banc X")
        ...
        for (int i = 1; i <= total; i++) {
            ...
            if (placesDuClient.contains(i)) {
                html.append("<div class='seat selected-blue' ")
                    ...
                    .append("style='").append(baseStyle).append(" background-color: #0d6efd; color: white; cursor: pointer; border: 2px solid #004299;' ")
            ...
            // CAS 2 : Place occupée par un autre client (GRISE)
                ...
                html.append("<div class='seat occupied-grey' ")
                    .append("data-num='").append(i).append("' ")
                    .append("style='").append(baseStyle).append(" background-color: #e9ecef; color: #adb5bd; cursor: not-allowed; border: 1px solid #dee2e6; pointer-events: none;' ")
            ...
            // CAS 3 : La place est LIBRE (BLANCHE)
                ...
                html.append("<div class='seat available-white' ")
                    .append("data-num='").append(i).append("' ")
                    .append("onclick='selectSeat(this, ").append(i).append(")' ")
                    .append("style='").append(baseStyle).append(" background-color: #ffffff; color: #1a3a5f; cursor: pointer; border: 2px solid #1a3a5f;' ")
