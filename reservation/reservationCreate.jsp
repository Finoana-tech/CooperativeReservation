...
<%
    ...
    // Récupération des paramètres pour le mode édition/pré-remplissage
    String idvoitParam = request.getParameter("idvoit");
    String idreservParam = request.getParameter("idreserv"); // Important pour différencier les places
%>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Réservation - Gestion Coopérative</title>
    ...
    <style>
        ...
        .btn-validate { background-color: var(--primary-color); color: white; border: none; padding: 15px; width: 100%; font-weight: bold; text-transform: uppercase; margin-top: 15px; border-radius: 4px; transition: 0.3s; }
        ...
        /* BLEU PLEIN : Sélectionné (actuel ou nouveau) */
    ...
    <script>
        let selectedSeats = [];
        const idReservCourante = "<%= (idreservParam != null) ? idreservParam : "" %>";
        ...
        function toggleAvance() {
            ...
            var inputAvance = document.getElementById("montant_avance_input");
        ...
        function loadAvailableSeats() {
            ...
            if (idvoit && date) {
                ...
                // URL adaptée pour inclure l'ID de réservation pour le mode modification
                var url = "${pageContext.request.contextPath}/CheckPlacesServlet?idvoit=" + idvoit + "&date=" + date + "&idreserv=" + idReservCourante;
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        // Initialiser selectedSeats avec les places déjà bleues (mode modif)
                        const preSelected = document.querySelectorAll('.selected-blue');
                        selectedSeats = Array.from(preSelected).map(s => {
                            // On extrait le numéro de place via l'appel onclick selectSeat(this, X)
                    ...
                    .catch(error => {
                        ...
                        document.getElementById("seatGridContainer").innerHTML = "<p class='text-danger text-center'>Erreur de chargement des places.</p>";
                    });
            }
        }
        ...
        function validerMontantAvance(event) {
            ...
            if (nbPlaces === 0) {
                ...
                Swal.fire({ title: 'Attention', text: 'Veuillez sélectionner au moins une place.', icon: 'warning' });
                return false;
            }

            const typePaiement = document.getElementById("payment").value;
            const avanceSaisie = parseInt(document.getElementById("montant_avance_input").value) || 0;
            ...
            if (typePaiement === "avec avance" && avanceSaisie > totalFrais) {
                ...
                Swal.fire({
                    ...
                    text: 'L\'avance (' + avanceSaisie + ' Ar) dépasse le total (' + totalFrais + ' Ar).',
                    icon: 'error'
                });
                return false;
            }
...
<div class="container">
    <div class="form-card">
        <form action="${pageContext.request.contextPath}/ReservationServlet" method="post" onsubmit="return validerMontantAvance(event)">
            <input type="hidden" name="action" value="<%= (idreservParam != null) ? "update" : "create" %>">
            <% if (idreservParam != null) { %>
                <input type="hidden" name="idreserv" value="<%= idreservParam %>">
            <% } %>

            <div class="mb-4">
                <label class="form-label"><i class="bi bi-person-fill"></i> Client</label>
                <select name="idcli" class="form-select shadow-sm" required>
                    <option value="">-- Sélectionner le client --</option>
                    <% for (Client c : clients) { %>
                        <option value="<%= c.getIdcli() %>">
            ...
            <div class="row">
                <div class="col-md-6 mb-4">
                    ...
                    <select name="idvoit" id="idvoitSelect" class="form-select shadow-sm" onchange="loadAvailableSeats()" required>
                        ...
                        <% for (Voiture v : voitures) { %>
                            <option value="<%= v.getIdvoit() %>" <%= (v.getIdvoit().equals(idvoitParam)) ? "selected" : "" %>>
                ...
                <div class="col-md-6 mb-4">
                    <label class="form-label"><i class="bi bi-calendar-check"></i> Date du voyage</label>
                    <input type="date" name="date_voyage" id="dateVoyage" class="form-control shadow-sm" onchange="loadAvailableSeats()" required>
                </div>
            </div>

            <div id="dynamicSeatArea" style="display:none;" class="mb-4">
                <label class="form-label text-primary"><i class="bi bi-grid-3x3-gap"></i> Plan des places :</label>
                ...
                <div class="d-flex gap-3 mt-2 mb-3 small">
                    ...
                    <div><span class="badge" style="background-color: #0d6efd;"> </span> Sélectionnée</div>
                    <div><span class="badge" style="background-color: #e9ecef; border: 1px solid #dee2e6; color:#adb5bd"> </span> Occupée</div>
                </div>

                <div class="mt-3 p-3 bg-light border-start border-primary border-4 rounded">
                    <span class="small text-muted text-uppercase fw-bold">Places choisies :</span>
                    <div id="displayPlaceNum" class="fw-bold text-primary fs-5 mt-1">Aucune</div>
                    <input type="hidden" name="place" id="inputPlace" value="" required>
                </div>
            </div>

            <div class="row">
                <div class="col-md-6 mb-3">
                    <label class="form-label"><i class="bi bi-cash-stack"></i> Type de Paiement</label>
                ...
                <div id="avanceDiv" class="col-md-6 mb-3" style="display:none;">
                    <label class="form-label">Montant à verser (Ar)</label>
                    <input type="number" name="montant_avance" id="montant_avance_input" class="form-control shadow-sm" value="0" min="0">
                </div>
            </div>

            <button type="submit" class="btn-validate shadow">
                <i class="bi bi-check-circle-fill"></i> <%= (idreservParam != null) ? "METTRE À JOUR" : "CONFIRMER LA RÉSERVATION" %>
            </button>
        </form>

        <div class="text-center mt-4">
            <a href="${pageContext.request.contextPath}/ReservationServlet?action=list" class="text-decoration-none text-muted small fw-bold">
                RETOUR AU JOURNAL
            </a>
        </div>
    </div>
...
<footer class="text-center">
    <div class="container">
        <strong>GESTION COOPÉRATIVE TRANSPORT &copy; 2026</strong>
    </div>
</footer>
